...
static void ST7789_WriteCommand(uint8_t cmd)
{
	ST7789_Select();
	ST7789_DC_Clr();
	hpm_spi_transmit_blocking(BOARD_LCD_SPI_BASE, &cmd,sizeof(cmd), 0xFFFFFFFF);
	ST7789_UnSelect();
}
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
	ST7789_DC_Set();
        ST7789_Select();
        hpm_spi_transmit_blocking(BOARD_LCD_SPI_BASE, buff, buff_size, 0xFFFFFFFF);
        ST7789_UnSelect();
}

static void ST7789_WriteDMAData(uint8_t *buff, size_t buff_size)
{
	ST7789_DC_Set();
        hpm_spi_transmit_nonblocking(BOARD_LCD_SPI_BASE, buff, buff_size);
}
...
static void ST7789_WriteSmallData(uint8_t data)
{
	ST7789_Select();
	ST7789_DC_Set();
	hpm_spi_transmit_blocking(BOARD_LCD_SPI_BASE, &data,sizeof(data), 0xFFFFFFFF);
	ST7789_UnSelect();
}
...
void ST7789_Init(void)
{
    ...
    {
            uint8_t data[] = {0x1F, 0x1F, 0x00, 0x33, 0x33};
            ST7789_WriteData(data, sizeof(data));
    }
    ...
    ST7789_WriteSmallData(0x44);			//	Default value
    ST7789_WriteCommand(0xBB);				//	VCOM setting
    ST7789_WriteSmallData(0x1D);			//	0.725v (default 0.75v for 0x20)
    ST7789_WriteCommand(0xC0);				//	LCMCTRL	
    ST7789_WriteSmallData (0x2C);			//	Default value
    ST7789_WriteCommand (0xC2);				//	VDV and VRH command Enable
    ST7789_WriteSmallData (0x01);			//	Default value
    ST7789_WriteCommand (0xC3);				//	VRH set
    ST7789_WriteSmallData (0x0F);			//	+-4.45v (defalut +-4.1v for 0x0B)
    ST7789_WriteCommand (0xC6);				//	Frame rate control in normal mode
    ST7789_WriteSmallData (0x13);			//	Default value (60HZ)
    ST7789_WriteCommand (0xD0);				//	Power control
    ST7789_WriteSmallData (0xA7);			//	Default value
    ST7789_WriteCommand (0xD0);				//	Power control
    ...
    {
        uint8_t data[] = {0xF0, 0x01, 0x06, 0x06, 0x09, 0x14, 0x34, 0x44, 0x4A, 0x3A, 0x14, 0x14, 0x29, 0x2E};
        ST7789_WriteData(data, sizeof(data));
    }

    ST7789_WriteCommand(0xE1);
    {
      uint8_t data[] = {0xF0, 0x0D, 0x10, 0x0D, 0x0B, 0x15, 0x33, 0x43, 0x4A, 0x2B, 0x17, 0x16, 0x2A, 0x2F};
      ST7789_WriteData(data, sizeof(data));
    }

    ST7789_WriteCommand(0xE4);
    {
      uint8_t data[] = {0x1D, 0x00, 0x00};
      ST7789_WriteData(data, sizeof(data));
    }
...
void ST7789_DrawDMAImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
	if ((x >= ST7789_WIDTH) || (y >= ST7789_HEIGHT))
		return;
	if ((x + w - 1) >= ST7789_WIDTH)
	...
	if ((y + h - 1) >= ST7789_HEIGHT)
        ...
        ST7789_SetAddressWindow(x, y, x + w - 1, y + h - 1);
	ST7789_Select();
	ST7789_WriteDMAData((uint8_t *)data, sizeof(uint16_t) * w * h);
}
