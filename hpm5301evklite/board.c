...
void board_timer_us_create(uint32_t us, board_timer_cb cb)
{
    ...
    gptmr_channel_config(BOARD_CALLBACK_TIMER, BOARD_CALLBACK_TIMER_CH, &config, false);
    gptmr_channel_reset_count(BOARD_CALLBACK_TIMER, BOARD_CALLBACK_TIMER_CH);
    gptmr_enable_irq(BOARD_CALLBACK_TIMER, GPTMR_CH_RLD_IRQ_MASK(BOARD_CALLBACK_TIMER_CH));
    intc_m_enable_irq_with_priority(BOARD_CALLBACK_TIMER_IRQ, 1);
...
{
    init_gpio_pins();
    gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_DO_GPIOA, 2, 1);
    gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_DO_GPIOA, 3, 0);
    gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_DO_GPIOA, 26, 0);
    gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_DO_GPIOA, 6, 0);
}

void board_init_key_pins(void)
{
    HPM_IOC->PAD[IOC_PAD_PA31].FUNC_CTL = IOC_PA31_FUNC_CTL_GPIO_A_31;
    HPM_IOC->PAD[IOC_PAD_PA31].PAD_CTL =  IOC_PAD_PAD_CTL_PRS_SET(2) | IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1);
...
void board_init_spi_pins_with_gpio_as_cs(SPI_Type *ptr)
{
    ...
    if (ptr == HPM_SPI0) {
        //gpio_set_pin_output_with_initial(BOARD_LCD_SPI_CS_GPIO_CTRL, GPIO_GET_PORT_INDEX(BOARD_LCD_SPI_CS_PIN),
        //                             GPIO_GET_PIN_INDEX(BOARD_LCD_SPI_CS_PIN), !BOARD_LCD_SPI_CS_ACTIVE_LEVEL);
    }
}
