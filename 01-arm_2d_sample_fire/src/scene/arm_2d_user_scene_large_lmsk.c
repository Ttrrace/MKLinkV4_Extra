                                                ...
                                                size_t tNumberOfWords)
{
    ...
    FRESULT res;
    size_t len = 0;
    DWORD fatfs_offset = wAddress - 0x90000000 ;
    static FIL *phFile = NULL;
    if(phFile == NULL){
        phFile = malloc(sizeof(FIL));
        f_open(phFile, "bad_apple.lmsk", FA_READ);
    }
    f_lseek(phFile, fatfs_offset);
    f_read(phFile, pwBuffer, tNumberOfWords * sizeof(uint32_t), &len);
