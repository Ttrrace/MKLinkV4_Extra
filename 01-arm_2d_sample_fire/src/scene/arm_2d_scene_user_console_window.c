...
#include "arm_2d_scene_user_console_window.h"
...
static void __console_box_printf(arm_2d_scene_t *ptScene,uint8_t *pchByte,uint16_t hwLen)
{
    ...
    for(uint16_t i = 0; i<hwLen;i++){
       ...
       console_box_printf(&this.tConsole, "%c",pchByte[i]);
    }
}
...
static void __on_scene_console_window_load(arm_2d_scene_t *ptScene)
{
    user_scene_console_window_t *ptThis = (user_scene_console_window_t *)ptScene;
    ARM_2D_UNUSED(ptThis);
    connect(&tUartMsgObj, SIGNAL(uart_sig), ptScene, SLOT(__console_box_printf));  
    connect(&tUartMsgObj, SIGNAL(uart_rx_time_sig), ptScene, SLOT(__console_box_printf));
    connect(&tUartMsgObj, SIGNAL(uart_tx_time_sig), ptScene, SLOT(__console_box_printf));
    for(uint32_t i = 0; i<(800 / 8)-(160 / 8);i++){
       console_box_putchar(&this.tConsole,'\n');
    }
}
...
static void __on_scene_console_window_depose(arm_2d_scene_t *ptScene)
{
    user_scene_console_window_t *ptThis = (user_scene_console_window_t *)ptScene;
    ARM_2D_UNUSED(ptThis);
    disconnect(&tUartMsgObj, SIGNAL(uart_sig), ptScene, SLOT(__console_box_printf));  
...
static void __on_scene_console_window_frame_start(arm_2d_scene_t *ptScene)
{
    ...
    //static uint16_t s_hwLineCount = 100;
    ...
    //int32_t iResult;
    //arm_2d_helper_time_cos_slider(-200, 0, 5000, 0, &iResult, &this.lTimestamp[1]);

    //this.iConsoleYScrolling = iResult;
...
IMPL_PFB_ON_DRAW(__pfb_draw_scene_console_window_handler)
{
    ...
    /*-----------------------draw the foreground begin-----------------------*/
        ...
        arm_2d_align_centre(__top_canvas, 240, 160) {
            ...
            draw_round_corner_box(  ptTile, 
                                    ...
                                    128);
            /* draw console */
            arm_2d_dock(__centre_region, 6) {
                arm_2d_container(ptTile, __console, &__dock_region) {

                        static int16_t offset = 0;
                        arm_2d_region_t tConsoleBox = {
                            .tLocation.iY = (offset + this.iY) * 8 - 800 + __console_canvas.tSize.iHeight,
                            .tSize = {240, 800},
                        };
                        if(tConsoleBox.tLocation.iY < (- 800 + __console_canvas.tSize.iHeight)){
                            offset = -this.iY;
                        }
                                        ...
                                        user_scene_console_window_t *ptThis)
{
    ...
    *ptThis = (user_scene_console_window_t){
    ...
    do {
        ...
        static uint8_t s_chConsoleBuffer[(240 / 6) * (800 / 8)];
